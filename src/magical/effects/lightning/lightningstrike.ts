...
type Segment = {
	...
	pos0: THREE.Vector3,
	pos1: THREE.Vector3,
	linPos0: THREE.Vector3,
	linPos1: THREE.Vector3,
	up0: THREE.Vector3,
	up1: THREE.Vector3,
	radius0: number,
	radius1: number,
...
export class LightningStrike extends THREE.BufferGeometry {
	constructor(public rayParameters: LightningParam) {
		super();
		// Set parameters, and set undefined parameters to default values
		rayParameters = rayParameters || {};
		this.init(this.copyParameters(rayParameters, rayParameters));
	...
	SIN30DEG = Math.sin(30 * Math.PI / 180);
	...
	copyParameters(dest: LightningParam, source: LightningParam) {

		source = source || {};
		dest = dest || {};

		const vecCopy = (v: THREE.Vector3) => {
			if (source === dest) {
				return v;
			} else {
				return v.clone();
	...
	update(time: number) {
		...
		if (this.rayParameters.isEternal || (
			this.rayParameters.birthTime && this.rayParameters.deathTime &&
			this.rayParameters.birthTime <= time && time <= this.rayParameters.deathTime)) {

			this.updateMesh(time);

			if (time < this.subrays[0].endPropagationTime) {
				this.state = this.RAY_PROPAGATING;
			}
			else if (time > this.subrays[0].beginVanishingTime) {
				this.state = this.RAY_VANISHING;
			}
			else {
				this.state = this.RAY_STEADY;
			}
			this.visible = true;
		}
		else {
			this.visible = false;
			if (this.rayParameters.birthTime && time < this.rayParameters.birthTime) {
				this.state = this.RAY_UNBORN;
			}
			else {
				this.state = this.RAY_EXTINGUISHED;
			}
		}
	};
	visible = false
	...
	randomGenerator?: any
	seedGenerator?: any
	onDecideSubrayCreation?: Function
	...
	init(rayParameters: LightningParam) {
		...
		// Internal state
		this.state = this.RAY_INITIALIZED;
	...
	forwardsFill?: THREE.Vector3;
	side?: THREE.Vector3;
	down?: THREE.Vector3;
	middlePos?: THREE.Vector3;
	middleLinPos?: THREE.Vector3;
	newPos?: THREE.Vector3;
	vPos?: THREE.Vector3;
	cross1?: THREE.Vector3;

	subrayConePosition?: Function
	subrayCylinderPosition?: Function
	...
	createMesh() {
		...
		if (!this.isStatic && this.index) {
			...
			if (this.generateUVs && this.uvsAttribute) {
				this.uvsAttribute.setUsage(THREE.DynamicDrawUsage)
			}
		}

		// Store buffers for later modification
		this.vertices = this.positionAttribute.array as Float32Array;
		if (this.index) this.indices = this.index.array as Uint32Array;
		if (this.generateUVs && this.uvsAttribute) {
			this.uvs = this.uvsAttribute.array as Float32Array;
		}
	...
	updateMesh(time: number) {
		...
		if (this.index) this.index.needsUpdate = true;
	...
	fillMesh(time: number) {
		...
		this.fractalRay(time, (segment: Segment) => {
	...
	initSubray(subray: any, rayParameters: LightningParam) {
	...
	fractalRay(time: number, segmentCallback: Function) {
	...
	fractalRayRecursive(segment: Segment) {
		...
		if (!this.newPos || !this.simplexX || !this.simplexY || !this.simplexZ ||
			!this.forwards || !this.middlePos || !this.middleLinPos || !this.cross1) {
			return
		}
		// Interpolation
	...
	createDefaultSubrayCreationCallbacks() {
		...
		this.onDecideSubrayCreation = (segment: Segment, lightningStrike: LightningStrike) => {
			...
			const period = lightningStrike.rayParameters.subrayPeriod ?? 0;
			const dutyCycle = lightningStrike.rayParameters.subrayDutyCycle ?? 0;
			...
			if (subray.recursion < lightningStrike.maxSubrayRecursion && lightningStrike.numSubrays < lightningStrike.maxSubrays && random1() < probability) {
				...
				childSubray.radius0 = segment.radius0 * (lightningStrike.rayParameters.radius0Factor ?? 0);
				childSubray.radius1 = Math.min((lightningStrike.rayParameters.minRadius ?? 0), segment.radius1 * (lightningStrike.rayParameters.radius1Factor ?? 0));
		...
		this.onSubrayCreation = (segment: Segment, parentSubray: Subray, childSubray: Subray, lightningStrike: LightningStrike) => {
	...
	copy(source: LightningStrike) {
		...
		this.init(this.copyParameters({}, source.rayParameters));

		return this;
	};

	cloneLightningStricke() {
		return new LightningStrike(this.copyParameters({}, this.rayParameters));
	};
}
