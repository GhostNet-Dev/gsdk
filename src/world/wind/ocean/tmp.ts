...
function init() {
  ...
  var supportsDepthTextureExtension = !!renderer.extensions.get('WEBGL_depth_texture');
  ...
  var noiseMap = loader.load('assets/texture/gPz7iPX.jpeg');
  var dudvMap = loader.load('assets/texture/hOIsXiZ.png');
  ...
  var waterfallMaterial = new THREE.ShaderMaterial({
    uniforms: THREE.UniformsUtils.merge([THREE.UniformsLib['fog'], waterfallUniforms]),
    vertexShader: document.getElementById('vertexShaderWaterfall').textContent,
    fragmentShader: document.getElementById('fragmentShaderWaterfall').textContent,
    fog: true
  });

  waterfall = new THREE.Mesh(
    new THREE.CylinderGeometry(1, 1, 8, 16, 1, true),
    waterfallMaterial
  );
  ...
  waterMaterial.uniforms.tDepth.value =
    supportsDepthTextureExtension === true ? renderTarget.depthTexture : renderTarget.texture;
  ...
  particleMaterial.onBeforeCompile = function(shader) {
    ...
    shader.vertexShader = shader.vertexShader.replace(
      ...
      ['vec3 transformed = vec3( position );', 'transformed.y += t * 0.25;', 'vT = t;'].join('\n')
    );
